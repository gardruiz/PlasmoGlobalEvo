...
#This script is designed to cut vcf files using samples list for different populations, the sample list directory should contain different folders with the sampleid for each population, each population folder should contain the samples IDS for the subpopulations as a txt file 
...
for sample_list_dir in "$parent_dir"/*/; do
    ...
    if [ -d "$sample_list_dir" ]; then
        ...
        for sample_list_file in "$sample_list_dir"/*.txt; do
            ...
            output_file="$output_dir/$sample_name"_"$gene_name.vcf.gz"
